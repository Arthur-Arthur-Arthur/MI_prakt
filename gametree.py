...
class GameState:
    def __init__(
        self,
        linijas: list[tuple[int, int]] = None,
        p1_punkti: int = 0,
        p2_punkti: int = 0,
    ):
        if linijas is None:
...
class Node:
    ...
    def children(self):
        ...
        if not esosas_linijas:
            ...
            izmantotie_punkti = set(
                [item for sublist in esosas_linijas for item in sublist]
            )
        ...
        for start_point in brivie_punkti:
            for end_point in brivie_punkti:
                ...
                new_nodes.append(
                    Node(
                        GameState(jaunas_linijas, p1_new, p2_new),
                        jaunais_limenis,
                        self.gameinfo,
                    )
                )
...
def MinMax(
    start_node: Node, depth: int, is_p1
):  # p1 is always maximizing, easier to remember this way though
...
def AlphaBeta(
    start_node: Node, depth: int, is_p1, alpha=-1e16, beta=1e16
):  # p1 is always maximizing, easier to remember this way though
    children = start_node.children()
    if depth == 0 or not children:
        return start_node.score()
